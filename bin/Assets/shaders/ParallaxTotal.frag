...
void main (void)
{
	...
	vec2 newTexCoordMoon = vTexCoord + ((heightMoon * 0.004 - 0.002) * vVecMoon.xy);				
	...
	float diffuseMoon = max( dot(lVecMoon, bumpMoon), 0.3 );		
	...
	if (iTypeLight == 1)
	{
		...
		vec2 newTexCoord = vTexCoord + ((height * 0.004 - 0.002) * vVec.xy);	
								
		vec4 base = texture2D(colorMap, newTexCoord);
		vec3 bump = normalize(texture2D(normalMap, newTexCoord).xyz * 2.0 - 1.0);
		bump = normalize(bump);
		
		float valueSpecular = texture2D(specularMap, vTexCoord).x ;  //Grey scale --> x = y = z		
		...
		{		
			...
			//----------------------------------------SPOT-----------------------------------------------------------------------
			vec4 final_color = (gl_FrontLightModelProduct.sceneColor * gl_FrontMaterial.ambient);	//LUNA			
